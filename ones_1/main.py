...
class TaskManager:
    ...
    def checkFolder(self):
        """
        检查config.yaml文件里的path文件夹是否创建，若没有则创建
        :return:
        """
        try:
            ...
            while self.event != False:
                ...
                if taskInfoList == []:  # 无处于进行中的任务
                    ...
                    for taskInfo in taskInfoList:
                        ...
                        ProjectTaskTransitions = self.ONES_API.getProjectTaskTransitions(task_uuid)
                        if ProjectTaskTransitions['related_testcase_plans'] == '' \
                                or ProjectTaskTransitions['AGV_IP'] == "":
                            self.logger.info('Task:{} Detection not associated with test plan or AGV_IP...'.format(
                                taskInfo['task_name']))
                            ...
                            if self.task == None or self.task != task_uuid and task_uuid not in self.taskQueue:
                                summary = ProjectTaskTransitions['summary'] # 工作项标题
                                relatedTestPlans = ProjectTaskTransitions['related_testcase_plans']  # 关联测试计划
                                AGV_IP = ProjectTaskTransitions['AGV_IP']
                                Device_IP = ProjectTaskTransitions['Device_IP']
                                if task_uuid not in self.taskQueue:
                                    self.testPlan = relatedTestPlans
                                    parameter = {
                                        "summary":summary,
                                        "task_uuid": task_uuid,
                                        'assign': ProjectTaskTransitions['assign'], # 任务负责人
                                    ...
                                    if self.initRobot(AGV_IP) == True and AGV_IP not in self.agv_ip:
                                        ...
                                        self.ONES_API.update_ProjectTask_Status(task_uuid, '已完成')  # 任务状态修改成已完成
                                        self.updateNotesAndProgress(task_uuid,'初始化AGV失败，请检查AGV是否正常或者使用正在运行车辆IP...',100)
    ...
    def runTestPlan(self, parameter):
        ...
        try:
            ...
            self.logger.info(f'Get the test plan:{self.testPlan} total number of test cases：{len(testCaseList)}')
            ...
            for testcase in testCaseList:
                ...
                if str(caseId) in caseDriveIdList:
                    ...
                    if result != None:  # 去除没有用例驱动情况
                        ...
                        caseUuid,caseResult = self.processCaseResult(result, testPlan, caseUuid, executorUuid, stepsUuid)
            ...
            self.dingtalk.sendNotification(parameter['summary'],TOTAL,PASS,PASSRate,FAILED,FAILEDRate,BROKEN,BROKENRate,SKIP,SKIPRate)
            ...
            del task_uuid,TOTAL,PASS,PASSRate,FAILED,FAILEDRate,BROKEN,BROKENRate,SKIP,SKIPRate
        except BaseException as e:
    ...
    def processCaseResult(self, result: dict, testPlan: str, caseUuid: str, executorUuid: str, stepsUuid: str):
        ...
        """
        try:
            caseResultInfo,caseResult,uploadInfo = result['resultInfo'],result['caseResult'],result['upload attachments']
            ...
            self.logger.info('updateTestCaseStatus:["caseUuid":{},"caseResult":{}]'.format(caseUuid,caseResult))
