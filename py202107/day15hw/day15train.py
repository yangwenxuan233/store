...
f1 = open(r"D:\pscode\demo\day15hw\用户数据.txt", mode="r+", encoding="ANSI")
...
sum = 0
for i in list([i.split(",") for i in [i.replace("\n", "") for i in f1.readlines()]]):
    db_write(i)
    sum += int(i[2])
...
f2 = open(r"D:\pscode\demo\day15hw\day15\大美女.jpg", mode="rb")
...
copy_file(r"D:\pscode\demo\day15hw\day15\大美女.jpg", r"D:\python\大美女.jpg")
...
class User():
    ...
    def read_data(self):
        file = open(r"D:\pscode\demo\day15hw\names.txt", mode="r+", encoding="utf-8")
        data = []
        lines = list([i.split(",") for i in [i.strip('\n') for i in file.readlines()]])
        for i in lines:
            if i[0] != "":
                data.append(i)
        file.close()
...
user.upload_image("俞敏洪", "123456", r"D:\pscode\demo\day15hw\day15", r"D:\pscode\demo\day15hw\image", r"\大美女.jpg")


# 5.现在有这样一个叫scores.txt的文件，里面有赫敏、哈利、罗恩、马尔福四个人的几次魔法作业的成绩。
# 但是呢，因为有些魔法作业有一定难度，教授不强制同学们必须上交，所以大家上交作业的次数并不一致。
file_read = open(r"D:\pscode\demo\day15hw\scores.txt", mode="r+", encoding="utf-8")
file_write = open(r"D:\pscode\demo\day15hw\scores_final.txt", mode="w+", encoding="utf-8")

data = list([i.split(" ") for i in [i.strip('\n') for i in file_read.readlines()]])
