...
class SRModbusTk(object):
    ...
    def __init__(self, ip):
        ...
        self.state_analysis = State_Analysis.ReadInputRegister()  # 状态解析初始化
        # log日志功能初始化
        ...
        self.logger = logging.getLogger('SR_Pymodbus')
    ...
    def modbus_tcp_connect(self):
        ...
        try:
            ...
            self.logger.error('modbus_tcp_connect ERROR:' + str(e))
            sys.exit(0)

    def read_register_status(self, starting_address, quantity_of_x):
        """
        ...
        try:
            ...
            self.logger.error('read_register_status ERROR: ' + str(e))
    ...
    def decode_uint16(self, value):
        ...
        try:
            ...
            self.logger.error('decode_uint16 ERROR:' + str(e))
    ...
    def decode_int16(self, value):
        ...
        try:
            ...
            self.logger.error('decode_int16 ERROR: ' + str(e))

    @staticmethod
    def decode_uint32(double_uint16: tuple):
        """
...
class SRPymodbus(object):
    ...
    def __init__(self, ip: str):
        ...
        self.ip = ip
        self.state_analysis = State_Analysis.ReadInputRegister()  # 状态解析初始化
        self.write_single_function = State_Analysis.WriteSingleCoil()  # 单个线圈功能组初始化
        # log日志功能初始化 当前路径下生成日志文件
        logging.basicConfig(level=logging.INFO, format='%(asctime)s - %(name)s - %(levelname)s -->> %(message)s',
                            datefmt='%Y %H:%M:%S',
                            filename='./Modbus.log',
                            filemode='w')
    ...
    def pymodbus_connect_tcp(self):
        ...
        try:
            ...
            if not ret:
                self.logger.error(f'Connection to {self.ip} failed')
                sys.exit()
        except Exception as e:
    ...
    def read_input_register_system_status(self):
        ...
        """
        try:
            system_status = self.state_analysis.system_status(self.read_input_register(30001, 1).decode_16bit_uint())  # 系统状态
    ...
    def read_input_register_pose_status(self):
        ...
        try:
            ...
            dict_pose_status = {
                ...
                'pose': {
                    "pose_x": pose_x / 1000,  # m
                    'pose_y': pose_y / 1000,  # m
                    'pose_yaw': pose_yaw
                },
                'real_time_speed': {
    ...
    def use_pose_positioning(self, x, y, yaw):
        """
        通过位姿进行定位
        :param x: x方向(mm)
        :param y: y方向(mm)
    ...
    def use_manual_control(self, Vx, Vy, Vyaw):
        """
    ...
    def use_forced_pose_positioning(self, x, y, yaw):
        """
        通过位姿强制定位
        :param x: x方向（mm）
        :param y: y方向（mm）
    ...
    def free_navigation_pose(self, x, y, yaw, no=0):
        """
        自由导航到位置
        :param x: x方向（mm）
        :param y: y方向（mm）
    ...
    def free_navigation_move_site(self, site, no=0):
        """
        自由导航移动到站点
        :param site: 站点
        :param no: 任务编号
    ...
    def run_action_task(self, id, parameter0, parameter1, no=0):
        """
...
if __name__ == "__main__":
    ...
    st = SRPymodbus(ips)
    # 初始位置的角度数据
    first_station_data = st.read_input_register_pose_status()
    ...
    while True:
        if n > 2000:
            break
        st.use_manual_control(x, y, rad)
        time.sleep(0.1)
        n += 1

    # 五圈后位置角度数据
    last_station_data = st.read_input_register_pose_status()
    print(last_station_data)
