...
class TestObstacle(unittest.TestCase):
    ...
    def test_param_allright(self, **kwargs):
        '''验证类型: 正常, 用例标题: 电池电压, 预期结果: 电压状态正常, 是否冒烟: 否。
        '''
        self.ip = kwargs['ip']
        self.modbus = Standard_Modbus_Api.SRPymodbus(self.ip)
        data = float(self.modbus.read_input_register_battery_status()['battery_voltage(V)'])
        self.logger.info('battery_voltage(V) ' + str(data))
