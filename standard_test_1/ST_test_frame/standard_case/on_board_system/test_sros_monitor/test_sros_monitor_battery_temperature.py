...
class TestObstacle(unittest.TestCase):
    ...
    def test_param_allright(self, **kwargs):
        '''验证类型: 正常, 用例标题: 电池温度, 预期结果: 温度状态正常, 是否冒烟: 否。
        '''
        self.ip = kwargs['ip']
        self.modbus = Standard_Modbus_Api.SRPymodbus(self.ip)
        data = float(self.modbus.read_input_register_battery_status()['battery_temperature(℃）'])
        self.logger.info('battery_temperature(℃) ' + str(data))
