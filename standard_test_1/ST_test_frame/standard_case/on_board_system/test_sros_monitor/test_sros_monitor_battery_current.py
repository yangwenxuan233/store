...
class TestObstacle(unittest.TestCase):
    ...
    def test_param_allright(self, **kwargs):
        '''验证类型: 正常, 用例标题: 实时电流, 预期结果: 实时电流状态正常, 是否冒烟: 否。
        '''
        self.ip = kwargs['ip']
        self.modbus = Standard_Modbus_Api.SRPymodbus(self.ip)
        data = float(self.modbus.read_input_register_battery_status()['battery_current(A)'])
        self.logger.info('battery_current(A) ' + str(data))
